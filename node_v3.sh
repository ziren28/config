...
# 强制重载所有 supervisor 进程（移除→重读→添加，确保配置刷新）
_reload_all_supervisor() {
    # 确保 supervisord 在运行
    if ! pgrep -x "supervisord" > /dev/null 2>/dev/null; then
        /usr/bin/supervisord -c /etc/supervisor/supervisord.conf 2>/dev/null || true
    fi
    # 停止并移除所有已注册的进程组
    for svc in $(supervisorctl status 2>/dev/null | awk '{print $1}'); do
        supervisorctl stop "$svc" 2>/dev/null || true
        supervisorctl remove "$svc" 2>/dev/null || true
    done
    # 重新读取配置并添加
...
[[proxies]]
name = "dashboard-${node_tag}-${dash_port}"
type = "tcp"
localIP = "127.0.0.1"
localPort = ${dash_port}
...
setup_all_supervisor_confs() {
    ...
    if [[ -z "$dash_port" ]]; then
        error "setup_all_supervisor_confs: dash_port 为空!"
        return 1
    fi
...
menu_deploy() {
    ...
    if [[ -f "$NODE_INFO_FILE" ]]; then
        source "$NODE_INFO_FILE"
        local _dash_port=${DASHBOARD_PORT:-${WEB_PORT:-$(( ${SOCKS_PORT:-0} - 1 ))}}
        echo -e "  状态: ${GREEN}已部署${NC}"
        echo -e "  Socks5: ${CYAN}${FRPS_ADDR}:${SOCKS_PORT}${NC}"
        echo -e "  Dashboard: ${CYAN}${FRPS_ADDR}:${_dash_port}${NC}"
    else
        echo -e "  状态: ${YELLOW}未部署${NC}"
    fi
    ...
    for svc in salad-frpc salad-gost salad-heartbeat sshd cf-3001 cf-ssh cf-debug dashboard; do
        total=$((total + 1))
        supervisorctl status "$svc" 2>/dev/null | grep -q RUNNING && running=$((running + 1)) || true
    done
    ...
    echo -e "  ${BOLD}7)${NC} 一键初始化/修复节点（输入端口即可）"
    echo -e "  ${BOLD}8)${NC} 卸载节点"
    echo -e "  ${BOLD}0)${NC} 返回主菜单"
    separator
...
deploy_node() {
    ...
    # 检查 dashboard 脚本
    if [[ ! -f "$DASHBOARD_BIN" ]]; then
        info "下载 Dashboard 脚本..."
        if check_cmd rclone; then
            local latest_backup
            latest_backup=$(rclone ls "${R2_BUCKET}/" 2>/dev/null | awk '{print $2}' | grep -v chromium | sort -r | head -1)
            if [[ -n "$latest_backup" ]]; then
                rclone copy "${R2_BUCKET}/${latest_backup}" /tmp/ 2>/dev/null
                tar xzf "/tmp/${latest_backup}" -C / usr/local/bin/server-dashboard.py 2>/dev/null || true
...
deploy_quick_init() {
    ...
    if [[ -n "$default_port" ]]; then
        read -r -p "Socks5 端口 [${default_port}]: " port
        port=${port:-$default_port}
    else
        read -r -p "Socks5 端口: " port
        [[ -z "$port" ]] && { error "端口不能为空"; return; }
    fi

    local dash_port=$((port - 1))
    local frps_addr="$DEFAULT_FRPS_ADDR"
    local frps_port="$DEFAULT_FRPS_PORT"
    local frps_token="$DEFAULT_FRPS_TOKEN"
    local proxy_user="$DEFAULT_PROXY_USER"
    ...
    # 安装 rclone + 配置 R2（dashboard 下载需要）
    if ! check_cmd rclone; then
        info "安装 rclone..."
        check_cmd unzip || apt-get install -y -qq unzip > /dev/null 2>&1
        curl -s https://rclone.org/install.sh | bash > /dev/null 2>&1 || true
    ...
    # 生成配置
    info "生成配置..."
    generate_frpc_toml "$frps_addr" "$frps_port" "$frps_token" "$NODE_TAG" "$port"
    generate_heartbeat_script "$NODE_ID" "$DEFAULT_BRAIN_URL" "$DEFAULT_REPORT_SECRET"
    write_node_info "$frps_addr" "$port" "$proxy_user" "$proxy_pass"
...
PROFILE
    mkdir -p /run/sshd /etc/ssh/sshd_config.d 2>/dev/null
    cat > "$SSHD_CONFIG" << 'EOF'
Port 22
PermitRootLogin yes
PubkeyAuthentication yes
